...
export default async function AdminVentasPage() {
  ...
  // 2. Fetch de datos unificados
  const [bookings, quotes, expiredStats] = await Promise.all([
  ...
  return (
    <div className="p-8 bg-background min-h-screen">
      ...
      <div className="flex justify-between items-end mb-8">
        <div>
          <h1 className="text-4xl font-heading font-black text-white tracking-tight">Centro de <span className="text-primary">Ventas</span></h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <Card className="bg-card/40 border-border/20 backdrop-blur-sm">
          ...
          <CardContent>
            <div className="text-3xl font-black text-white">{MXN(potentialLoss)}</div>
      ...
      <Tabs defaultValue="bookings" className="space-y-6">
        <TabsList className="bg-card/50 border border-white/5 p-1 h-12 rounded-xl">
          <TabsTrigger value="bookings" className="gap-2 px-6 rounded-lg data-[state=active]:bg-primary data-[state=active]:text-white">
            <ShoppingBag className="w-4 h-4" /> Bookings (Web)
          </TabsTrigger>
          <TabsTrigger value="cotizaciones" className="gap-2 px-6 rounded-lg data-[state=active]:bg-primary data-[state=active]:text-white">
            <FileText className="w-4 h-4" /> Cotizaciones (Manual)
          </TabsTrigger>
          <TabsTrigger value="contratos" className="gap-2 px-6 rounded-lg data-[state=active]:bg-green-600 data-[state=active]:text-white">
            <CheckCircle2 className="w-4 h-4" /> Contratos
          </TabsTrigger>
        </TabsList>

        <TabsContent value="bookings" className="space-y-4">
          <VentasTableClient items={webBookings} />
        </TabsContent>
        ...
        <TabsContent value="cotizaciones" className="space-y-4">
          <div className="space-y-6">
            <div className="bg-primary/5 border border-primary/20 p-4 rounded-xl flex items-center gap-4">
              ...
              <p className="text-sm text-gray-300">Aquí se listan las cotizaciones creadas manualmente por el equipo de Vendetta y registros legacy (Quotes).</p>
            </div>
            <VentasTableClient items={manualQuotes} />
            {quotes.length > 0 && (
              <div className="mt-8">
...
function ContratosGrid({ items }: { items: any[] }) {
  if (items.length === 0) return (
    <div className="py-20 text-center border-2 border-dashed border-white/5 rounded-3xl">
      <p className="text-muted-foreground">Aún no hay contratos generados (eventos confirmados).</p>
    </div>
  )
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {items.map(c => (
        <Card key={c.id} className="bg-card/40 border-border/20 group hover:border-green-500/30 transition-all">
          <CardHeader className="pb-2">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="pt-2 border-t border-white/5 flex items-center justify-between">
              <div className="text-sm font-black text-white">{MXN(c.baseAmount + (c.viaticosAmount || 0))}</div>
              <Button 
                size="sm" 
                variant="outline" 
                className="h-8 gap-2 border-green-600/30 text-green-400 hover:bg-green-600 hover:text-white" 
                render={
