...
import { Inbox, MessageSquare, AlertCircle, Clock, User, Phone, CheckCircle2, Filter, History } from "lucide-react"
    ...
    <div className="p-8 max-w-6xl mx-auto space-y-8 pb-24">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 text-primary mb-2">
            ...
            <span className="text-sm font-bold uppercase tracking-widest text-blue-600">Atención al Cliente</span>
          </div>
          <h1 className="text-4xl font-heading font-bold text-foreground tracking-tight">Bandeja de Comunicación</h1>
      ...
      <Tabs defaultValue={params.tab === "log" ? "log" : "pendientes"} className="space-y-6">
        <TabsList className="flex flex-row flex-wrap bg-muted/50 p-1 border border-border/40 rounded-xl justify-start w-full">
          <TabsTrigger value="pendientes" className="shrink-0  snap-center rounded-lg gap-2 data-[state=active]:bg-background data-[state=active]:shadow-sm">
            <Inbox className="w-4 h-4" /> Pendientes de Atención
          </TabsTrigger>
          <TabsTrigger value="log" className="shrink-0  snap-center rounded-lg gap-2 data-[state=active]:bg-background data-[state=active]:shadow-sm">
            <History className="w-4 h-4 text-blue-600" /> Log de WhatsApp (Hoy)
          </TabsTrigger>
        </TabsList>

        <TabsContent value="pendientes" className="outline-none">
        ...
        <TabsContent value="log" className="outline-none">
          <Card className="bg-card border-border/40 overflow-hidden rounded-2xl">
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-border/40 bg-blue-600/5">
                    <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-blue-600">Fecha y Hora</th>
                    <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-blue-600">Número</th>
                    <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-blue-600">Intención</th>
                  ...
                  {incomingMessages.length === 0 ? (
                    <tr>
                      <td colSpan={4} className="px-6 py-12 text-center text-muted-foreground italic">
                        No hay mensajes registrados hoy.
                      </td>
                    </tr>
                  ) : (
                    incomingMessages.map((notif) => (
                      <tr key={notif.id} className="hover:bg-blue-600/5 transition-colors group">
                        <td className="px-6 py-4 text-sm font-medium text-foreground ">
                          {formatDateTimeMX(notif.createdAt)}
                        </td>
                        <td className="px-6 py-4 text-sm text-muted-foreground  font-mono">
                          {notif.recipient || "—"}
                        </td>
                        <td className="px-6 py-4 ">
                          <Badge variant="outline" className="text-[10px] font-black uppercase tracking-tighter bg-blue-500/10 text-blue-600 border-blue-500/20">
          ...
          <div className="mt-4 p-4 bg-muted/30 rounded-xl border border-dashed border-border/40 text-center">
            <p className="text-xs text-muted-foreground">
              Este es el log completo de entrada. Los mensajes que requieren atención se mueven automáticamente a la pestaña de "Pendientes".
            </p>
          </div>
        </TabsContent>
      </Tabs>
