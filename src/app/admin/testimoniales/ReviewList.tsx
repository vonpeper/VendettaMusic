...
export function ReviewList({ initialReviews }: { initialReviews: Review[] }) {
  ...
  return (
    <div className="space-y-12">
      ...
      {pending.length > 0 && (
        <section>
          <div className="flex items-center gap-3 mb-6">
             ...
             <h2 className="text-xl font-black text-white uppercase tracking-tight">Pendientes de Revisión</h2>
          </div>
      ...
      {/* El Resto */}
      <section>
        <h2 className="text-xl font-black text-white/40 uppercase tracking-tight mb-6">Historial y Aprobados</h2>
...
function ReviewCard({ review, loading, onApprove, onReject, onDelete }: any) {
  return (
    <div className={`group relative bg-card/40 border border-white/5 rounded-[2rem] p-6 transition-all duration-300 ${
      review.status === "pending" ? "border-primary/20 bg-primary/5 shadow-xl shadow-primary/5" : ""
    }`}>
      <div className="flex justify-between items-start mb-4">
        <div className="flex gap-1">
          {[...Array(5)].map((_, i) => (
            <Star key={i} className={`w-3 h-3 ${i < review.stars ? "fill-amber-500 text-amber-500" : "text-white/10"}`} />
          ))}
        </div>
        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          {onReject && (
            <button 
              disabled={loading}
              onClick={onReject}
              className="p-2 rounded-full bg-yellow-500/10 text-yellow-500 hover:bg-yellow-500 hover:text-white transition-all"
              title="Rechazar"
            >
              <X className="w-4 h-4" />
            </button>
      ...
      <p className="text-sm font-medium text-white mb-4 leading-relaxed italic">"{review.text}"</p>
      
      <div className="flex items-center gap-3 pt-4 border-t border-white/5">
        <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-xs font-black text-primary">
          {review.name.charAt(0).toUpperCase()}
        </div>
        <div>
          <div className="text-xs font-black text-white uppercase tracking-tight">{review.name}</div>
