...
export function BandEventForm({ onClose, editing }: any) {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
        <div className="bg-card border border-green-500/30 rounded-2xl p-8 max-w-sm w-full text-center shadow-2xl">
          ...
          <h3 className="text-xl font-bold text-white mb-2">{state.message || (isEditing ? "Actualizado" : "Creado")}</h3>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-card border border-white/10 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[92vh] overflow-y-auto">
        ...
        {state && !state.success && (
          <div className="mx-6 mt-4 p-3 bg-destructive/20 border border-destructive/40 rounded-lg flex items-center gap-3 text-sm text-white">
        ...
        <form action={formAction} className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2 md:col-span-2">
                ...
                <Input id="customName" name="customName" required defaultValue={editing?.customName || editing?.clientName}
                  placeholder="Ej. María González, Empresa XYZ..." className="bg-background border-white/10" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="date">Fecha *</Label>
                <Input id="date" name="date" type="date" required
                  defaultValue={editing?.eventDate || editing?.date} className="bg-background border-white/10 text-white" />
              </div>
              <div className="space-y-2">
                ...
                <select id="ceremonyType" name="ceremonyType" defaultValue={editing?.ceremonyType || editing?.eventType || "show"}
                  className="flex h-10 w-full rounded-md border border-white/10 bg-background px-3 py-2 text-sm text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring capitalize">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Input id="locationFree" name="locationFree" defaultValue={editing?.location || ""}
                    placeholder="Ej. Hacienda San José..." className="bg-background border-white/10 flex-1" />
                  <select 
                    className="bg-background border border-white/10 rounded-md px-2 text-xs text-white max-w-[140px]"
                    onChange={(e) => {
              ...
              <div className="space-y-2">
                ...
                <select id="status" name="status" defaultValue={editing?.status || "completed"}
                  className="flex h-10 w-full rounded-md border border-white/10 bg-background px-3 py-2 text-sm text-white capitalize">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input id="totalIncome" name="totalIncome" type="number" step="0.01"
                  ...
                  className="bg-background border-white/10 font-bold text-primary" />
              </div>
            </div>
          </fieldset>
          ...
          <fieldset className="space-y-4 border-t border-white/10 pt-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input id="paymentRef" name="paymentRef" defaultValue={editing?.paymentRef || ""} className="bg-background border-white/10" />
              </div>
            </div>
          </fieldset>
          ...
          <fieldset className="border-t border-white/10 pt-4">
            ...
            <textarea id="musicianNotes" name="musicianNotes" rows={2} defaultValue={editing?.notes}
              className="flex w-full rounded-md border border-white/10 bg-background px-3 py-2 text-sm text-white" />
          </fieldset>
