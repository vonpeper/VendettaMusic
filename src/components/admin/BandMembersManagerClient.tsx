...
export function BandMembersManagerClient({ members }: { members: PublicBandMember[] }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {!isCreating && !editing && (
        <Button onClick={() => setIsCreating(true)} className="bg-primary text-white hover:bg-red-700 font-bold uppercase text-xs tracking-widest gap-2">
            ...
            <form action={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input name="name" defaultValue={editing?.name} placeholder="Ej. Mauricio" required className="bg-white/5 border-white/10" />
                </div>
                <div className="space-y-2">
                  <label className="text-xs uppercase tracking-widest font-bold text-primary">Rol / Puesto</label>
                  <Input name="role" defaultValue={editing?.role} placeholder="Ej. Guitarra Líder" required className="bg-white/5 border-white/10" />
                </div>
                <div className="space-y-2">
                  <label className="text-xs uppercase tracking-widest font-bold text-primary">Emoji Representativo</label>
                  <Input name="emoji" defaultValue={editing?.emoji || "🎸"} required className="bg-white/5 border-white/10" />
                </div>
                <div className="space-y-2">
                  <label className="text-xs uppercase tracking-widest font-bold text-primary">Ruta de Foto Oficial</label>
                  <Input name="img" defaultValue={editing?.img || "/images/musicians/default.jpg"} required className="bg-white/5 border-white/10" />
                </div>
                <div className="space-y-2">
                  <label className="text-xs uppercase tracking-widest font-bold text-primary">Link Instagram (Opcional)</label>
                  <Input name="ig" defaultValue={editing?.ig || ""} placeholder="https://instagram.com/..." className="bg-white/5 border-white/10" />
                </div>
                <div className="space-y-2">
                  <label className="text-xs uppercase tracking-widest font-bold text-primary">Orden de Aparición</label>
                  <Input name="order" type="number" defaultValue={editing?.order || 0} required className="bg-white/5 border-white/10" />
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-xs uppercase tracking-widest font-bold text-primary">Mini-Biografía (Tarjeta)</label>
                <Textarea name="shortBio" defaultValue={editing?.shortBio} required rows={2} className="bg-white/5 border-white/10 resize-none" />
              </div>
              
              <div className="space-y-2">
                <label className="text-xs uppercase tracking-widest font-bold text-primary">Biografía Extendida (Modal)</label>
                <Textarea name="fullBio" defaultValue={editing?.fullBio} required rows={3} className="bg-white/5 border-white/10 resize-none" />
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {members.map((m) => (
          <div key={m.id} className="relative rounded-2xl border border-white/10 bg-white/5 overflow-hidden group">
            <div className="absolute top-3 right-3 flex gap-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
              <Button size="icon" variant="secondary" className="w-8 h-8 bg-black/50 hover:bg-primary/80 backdrop-blur-md" onClick={() => setEditing(m)}>
                 <Pencil className="w-3 h-3" />
              </Button>
              <Button size="icon" variant="destructive" className="w-8 h-8" onClick={() => handleDelete(m.id)} disabled={loading}>
            ...
            <div className="p-4">
               <p className="text-xs text-gray-400 italic line-clamp-3">{m.shortBio}</p>
            </div>
          </div>
        ))}
      </div>
