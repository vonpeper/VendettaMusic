...
export default function Step1_Paquete({ packages, data, onNext }: Props) {
  ...
  const isCustom = selectedPkg === "arma-tu-show" || 
                   selectedPkg === "custom" || 
                   selectedPkg === "personalizado" ||
                   (pkg?.name.toLowerCase().includes("armar") || false) || 
                   (pkg?.name.toLowerCase().includes("arma") || false) ||
                   (pkg?.name.toLowerCase().includes("custom") || false) ||
                   (pkg?.name.toLowerCase().includes("personal") || false)
  ...
  return (
    <div className="space-y-8">
      ...
      {isCustom && (
        <div className="space-y-8 p-6 bg-white/5 border border-white/10 rounded-[2.5rem] animate-in fade-in slide-in-from-bottom-4 duration-500">
          
          {/* Invitados y Audio logic */}
          <div className="space-y-4">
            <label className="text-xs font-black text-white uppercase tracking-widest flex items-center gap-2">
              <Users className="w-4 h-4 text-primary" /> Audio por Invitados
            </label>
            ...
            <p className="text-[11px] text-gray-400">
              {guests <= 100 ? "✓ Audio estándar incluído." : 
               guests <= 300 ? "⚠️ Requiere upgrade de audio (+$7,500)." : 
               "🔥 Requiere audio a nivel festival (+$10,000)."}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                <label className="text-xs font-black text-white uppercase tracking-widest flex items-center gap-2">
                  <Clock className="w-4 h-4 text-primary" /> Horas de Vendetta (Mín 2h)
                </label>
                <div className="flex items-center gap-3">
                  <Button variant="ghost" size="icon" onClick={() => setBandHrs(Math.max(2, bandHrs - 1))} className="h-8 w-8 rounded-full border border-white/10">-</Button>
                  <span className="text-xl font-black text-white w-8 text-center">{bandHrs}h</span>
                  <Button variant="ghost" size="icon" onClick={() => setBandHrs(bandHrs + 1)} className="h-8 w-8 rounded-full border border-white/10">+</Button>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-3">
                  <Button variant="ghost" size="icon" onClick={() => setDjHrs(Math.max(0, djHrs - 1))} className="h-8 w-8 rounded-full border border-white/10">-</Button>
                  <span className="text-xl font-black text-white w-8 text-center">{djHrs}h</span>
                  <Button variant="ghost" size="icon" onClick={() => setDjHrs(djHrs + 1)} className="h-8 w-8 rounded-full border border-white/10">+</Button>
                </div>
                {djHrs > 0 && (
                  <label className="flex items-center gap-2 cursor-pointer mt-2 group">
                    <input type="checkbox" checked={djTvs} onChange={e => setDjTvs(e.target.checked)} className="w-4 h-4 accent-red-600 rounded" />
                    <span className="text-xs text-gray-400 group-hover:text-white transition-colors">Añadir 2 TVs de 45" (+$700/h extra)</span>
                  </label>
                )}
              </div>
            </div>
            ...
            <div className="space-y-4">
              <label className="text-xs font-black text-white uppercase tracking-widest flex items-center gap-2">
                <Box className="w-4 h-4 text-primary" /> Equipamiento Extra
              </label>
                 ...
                 {[
                  ...
                  <button 
                    ...
                    className={`w-full flex items-center justify-between p-3 rounded-xl border transition-all ${
                      ex.state ? "border-primary bg-primary/10" : "border-white/5 bg-black/20"
                    }`}
                  >
                    <span className="text-xs font-bold text-white">{ex.label}</span>
                    <span className="text-[10px] text-primary font-black">+${ex.price}</span>
                  </button>
                 ))}
                 <div className="w-full flex items-center justify-between p-3 rounded-xl border border-white/5 bg-black/10 opacity-50 grayscale cursor-not-allowed">
                    <span className="text-xs font-bold text-gray-500">Pantalla LED 3x2</span>
                    <span className="text-[9px] text-gray-500 uppercase tracking-tighter">Próximamente</span>
                 </div>
              </div>
            </div>
          </div>
